...
# consistent character variables.
  ...
  mutate(approval_pres = case_when(approval_pres == 1 ~ "Strongly Approve",
  ...
  mutate(approval_gov = case_when(approval_gov == 1 ~ "Strongly Approve",
  ...
  mutate(econ = case_when(econ == 1 ~ "Gotten Much Better",
  ...
  mutate(news = case_when(news == 1 ~ "Most Of The Time",
  ...
  mutate(marstat = case_when(marstat == 1 ~ "Married",
                          marstat == 2 ~ "Separated",
                          marstat == 3 ~ "Divorced",
                          marstat == 4 ~ "Widowed",
