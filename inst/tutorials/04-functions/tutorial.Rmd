...
We want to call `add_rvs()` twenty times. That is a lot of manual function calling! Let's have the function `replicate()` do the work for us instead. Call `add_rvs()` twenty times using `replicate()`. Use the default value of the `n` argument in `add_rvs()`.
