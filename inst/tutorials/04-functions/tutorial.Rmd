...
Some of these exercise were taken from the collection of [RStudio Primers](https://rstudio.cloud/learn/primers), a great resource for practicing your skills.
...
We create a simple tibble with one variable. Add to the pipe some code which creates a new list-column called `col_2` by using `str_split()` to split the phrases into "Government", "Data Science", "Preceptor", and "Primer".
...
Now append `str()` to the end of the pipe in order to to read the tibble contents. Learning how a list-column is constructed is a necessary step in learning how to manipulate it's contents.
...
Great! Now that we've oriented ourselves to the dataset, create a list-column of `weight`s using `mutate()`called `weight_groups`, grouping by `Diet` and `Time`.
...
Use a `map_*` function  and `mutate()` to add a new column, `mean_weight`, which is the mean weight per row. Recall that each row corresponds to a unique combination of `Diet` and `Time`. As always, you should start by copy-pasting from the last answer.
...
First, create a list-column named `gdpPercap_yearly` that lists the `gdpPercap` per year grouping by continent.
...
```{r clean-5, eval=FALSE, echo= TRUE}
clean <- function(x){
  stopifnot(!is.null(x))
  if(x == -99 || x == ".") return(NA)
  x
...
Recall the function we wrote in the chapter to model a crooked game of craps. `crooked_craps()` is expected to return a winning 7 or 11 only *half* of the time the sum is actually 7 or 11. Instead, the other half of the time, it is supposed to return a 2.
