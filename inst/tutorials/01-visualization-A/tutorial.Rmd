...
<!-- BG: hint here is a little vague/ not really needed -->
...
<!-- BG: This is an example of something that wasn't in the primer. A potential description of what one is and the function ("list()") may be helpful before the vectors section. -->
...
```{r quiz-5, echo = FALSE}
quiz(caption = "Quiz - `qscores` data set",
  question("Which of the following is an identification variable?",
...
```
<!-- BG: nothing in the primer mentions "identification variable"  -->
## Grammar of graphics
...
### Scatterplot with the `qscores` data set
...
Let's keep playing with the `qscores` data set in the `PPBDS.data` package. We have filtered the data set using the `tidyverse` package based on the five departments with the classes with the most enrollment: ECON, COMPSCI, STAT, GOV, and MATH. The filtered data set is called `top_five_qscores`.
...
Using the `top_five_qscores` data set and `ggplot()`, make a boxplot where `department` is on the x-axis and `rating` is on the y-axis.
...
Using the `trains` data set, create a boxplot with `party` on the x-axis and `att_start` on the y-axis.

<!-- BG: I think this can be taken out for repetition sake. There are three consecutive examples with three different data sets doing the same thing and this question has no follow up questions, unlike the one following it.  -->
...
Using the data set `governors`, create a boxplot where `party` is on the x-axis and `win_margin` is on the y-axis.

```{r box-I-3, exercise=TRUE, exercise.lines=10}
...
```{r box-I-4, exercise=TRUE, xercise.lines=10}
...
Great! Now let's make the outliers a little bit easier to see. Set `outlier.size` to 4, and `outlier.color` to "red".

<!-- BG: These outlier arguments are not mentioned in the primer. I think the question should include the function it is placed in directly in the question rather than the hint. -->
...
Put `outlier.size` and `outlier.color` inside of geom_boxplot().
```
...
<!-- DK: Some words about violin plots and why we use them. -->
...
Great! Now try doubling the bandwidth using the `adjust` argument in `geom_violin()`.

<!-- BG: The adjust argument is not mentioned in primer. I had to look up that you set adjust=2 online.  -->
...
<!-- BG: the 'kernel' argument is not mentioned in the primer. Brief explanation of its purpose in the question may be helpful.  -->
...
<!-- BG: may be helpful to add "in geom_smooth()"at the end. -->
