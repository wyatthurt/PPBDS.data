...
It seems that there are 1,500 beads in the urn (i.e. 1,500 rows in our tibble), but is this really the case? Run `nrow()` on `urn` to figure out how many beads are really in the urn.

```{r shovel2, exercise = TRUE, exercise.setup = "urn"}

```

###

Using dplyr functions, it would be pretty easy to find the exact number of red beads in the urn. However, this is not always possible in the real world. To simulate the process of real-world sampling, pipe `urn` into `rep_sample_n()` with a shovel `size` of 20 (i.e. the sample should have 20 observations).

```{r shovel3, exercise = TRUE, exercise.setup = "urn"}
...
Continuing your pipe, `mutate()` two new variables: `is_red` should be a logical equal to the result of asking if `color` is "red," and `is_white` should be identical but replaced with the `color` "white." Remember to copy and paste your code from the previous exercise.

```{r shovel4, exercise = TRUE, exercise.setup = "urn"}
...
Next, use `summarize()` to create two more variables. `num_red` should be equal to the `sum()` of the `is_red` column, and `num_white` should be equal to the `sum()` of the `is_white` column. Read the textbook to understand how and why this works.

```{r shovel5, exercise = TRUE, exercise.setup = "urn"}
...
```{r shovel6, exercise = TRUE, exercise.setup = "urn"}
...
```{r shovel7, exercise = TRUE, exercise.setup = "urn"}

```

###

Nice work! Running the code below gives us a way to virtually sample our urn in 20-bead increments. Notice that running the code multiple times gives us different estimates for the number of red beads in the urn.

Therefore, our next step will be to use our "virtual shovel" 50 times and create a distribution of the results. Start by changing the `reps` argument of `rep_sample_n()` to 50 and its `replace` argument to `TRUE` (this is equivalent to sampling with replacement, which is what we want).

```{r shovel8, exercise = TRUE, exercise.setup = "urn", exercise.lines = 7}
urn %>%
  rep_sample_n(size = 20) %>%
  mutate(is_red = (color == "red"), is_white = (color == "white")) %>%
  summarize(num_red = sum(is_red), num_white = sum(is_white)) %>%
...
```{r shovel9, exercise = TRUE, exercise.setup = "urn"}
...
```{r shovel10, exercise = TRUE, exercise.setup = "urn"}
...
```{r shovel11, exercise = TRUE, exercise.setup = "urn", exercise.lines = 10}
urn %>%
...
```{r shovel12, exercise = TRUE, exercise.setup = "urn"}
...
To finish this graph, use `labs()` to rename the x-axis label to "Estimated number of red beads based on proportion of sample." In addition, give the graph a suitable title of your choice.

```{r shovel13, exercise = TRUE, exercise.setup = "urn", exercise.lines = 11}
urn %>%
  rep_sample_n(size = 20, reps = 50, replace = TRUE) %>%
  mutate(is_red = (color == "red"), is_white = (color == "white")) %>%
  summarize(num_red = sum(is_red), num_white = sum(is_white)) %>%
...
In this section, we're going to analyze the effect of increased sample size and increased number of samples through purrr's `map()` family of functions.
...
In the code chunk below, write a function `mean_squares()` that calculates the `mean()` of the squares of two numbers, `x` and `y`. Then, use your function to calcuate the mean of $6^2$ and $14^2$. Remember that `mean()` takes in a vector as its input.
...
```{r map3, exercise = TRUE, exercise.setup = "urn", exercise.lines = 7}
urn %>%
  rep_sample_n(size = 20, reps = 50, replace = TRUE) %>%
  mutate(is_red = (color == "red"), is_white = (color == "white")) %>%
  summarize(num_red = sum(is_red), num_white = sum(is_white)) %>%
...
```{r map4, exercise = TRUE, exercise.setup = "urn"}
...
Pipe your tibble into a `mutate()` statement to create the new variable `sample_results`. For now, set `sample_results` equal to the result of a `sample_urn()` call with `size` equal to `sample_size` and `reps` equal to `num_reps`. Try to figure out why you receive an error.

```{r map5, exercise = TRUE, exercise.setup = "urn"}
...
```{r map6, exercise = TRUE, exercise.setup = "urn"}
...
```{r map7, exercise = TRUE, exercise.setup = "urn"}
...
```{r map8, exercise = TRUE, exercise.setup = "urn"}
...
```{r map9, exercise = TRUE, exercise.setup = "urn", exercise.lines = 12}
...
tibble(sample_size = c(20, 20, 100, 100), num_reps = c(50, 100, 50, 100)) %>%
  mutate(sample_results = map2(sample_size, num_reps, ~ sample_urn(size = .x, reps = .y)))
```
...
```{r map10, exercise = TRUE, exercise.setup = "urn"}
...
```{r map11, exercise = TRUE, exercise.setup = "urn"}
...
```{r map12, exercise = TRUE, exercise.setup = "urn"}
...
```{r map13, exercise = TRUE, exercise.setup = "urn"}
...
```{r map14, exercise = TRUE, exercise.setup = "urn", exercise.lines = 16}
sample_urn <- function(size, reps) {
  urn %>%
    rep_sample_n(size = size, reps = reps, replace = TRUE) %>%
    mutate(is_red = (color == "red"), is_white = (color == "white")) %>%
...
In the code cunk below, run `sd()` (the function that returns standard deviation) on the vector `(1, 2, 3, 4, 5)`.
...
This time, calculate the standard deviation of the vector `(143, 144, 145, 146, 147)`. See if you can predict what the result will be before you run the code.
...
Calculate the standard deviation of the vector `(2, 3, 4)` and compare the result to the results from the previous two exercises.
...
Calculate the standard deviation of the vector `(7, 7, 7, 7, 7)`. See if you can predict the result before you run the code.
...
Finally, calculate the standard deviation of `(1, 1, 1, 5, 5)` and compare the result to the result of the first exercise.
...
```{r sd6, exercise = TRUE, exercise.setup = "urn", message = FALSE, exercise.lines = 13}
sample_urn <- function(size, reps) {
  urn %>%
    rep_sample_n(size = size, reps = reps, replace = TRUE) %>%
    mutate(is_red = (color == "red"), is_white = (color == "white")) %>%
...
tibble(sample_size = seq(from = ..., to = ..., by = ...), num_reps = c(50, 100, 50, 100))
```
...
```{r sd7, exercise = TRUE, exercise.setup = "urn", message = FALSE}

```

```{r sd7-hint, eval = FALSE}
tibble(sample_size = c(..., seq(...)), num_reps = c(50, 100, 50, 100))
```
...
```{r sd9, exercise = TRUE, exercise.setup = "urn", message = FALSE, exercise.lines = 13}
sample_urn <- function(size, reps) {
  urn %>%
    rep_sample_n(size = size, reps = reps, replace = TRUE) %>%
    mutate(is_red = (color == "red"), is_white = (color == "white")) %>%
...
tibble(sample_size = c(1:5, seq(from = 6, to = 40, by = 2)), num_reps = c(50, 100, 50, 100)) %>%
  mutate(sample_results = map2(sample_size, num_reps, ~ sample_urn(size = .x, reps = .y))) %>%
  unnest(cols = sample_results)
```

```{r sd9-hint, eval = FALSE}
tibble(sample_size = c(1:5, seq(from = 6, to = 40, by = 2)), num_reps = ...)
```
...
```{r sd10, exercise = TRUE, exercise.setup = "urn", message = FALSE}
...
```{r sd11, exercise = TRUE, exercise.setup = "urn", message = FALSE}
...
```{r sd12, exercise = TRUE, exercise.setup = "urn", message = FALSE}
...
```{r sd13, exercise = TRUE, exercise.setup = "urn", message = FALSE}
...
```{r sd14, exercise = TRUE, exercise.setup = "urn", message = FALSE, exercise.lines = 20}
sample_urn <- function(size, reps) {
  urn %>%
    rep_sample_n(size = size, reps = reps, replace = TRUE) %>%
    mutate(is_red = (color == "red"), is_white = (color == "white")) %>%
...
tibble(sample_size = c(1:5, seq(from = 6, to = 40, by = 2)), num_reps = rep(50, 23)) %>%
  mutate(sample_results = map2(sample_size, num_reps, ~ sample_urn(size = .x, reps = .y))) %>%
  unnest(cols = sample_results) %>%
...
tibble(sample_size = c(1:5, seq(from = 6, to = 40, by = 2)), num_reps = rep(...))
```
...
rep_sample_n(tibble(...), size = ..., reps = ..., replace = ...)
```

###

Pipe your `rep_sample_n()` statement into `group_by()` to group the tibble by the `replicate` variable.
...
Using `summarize()`, create the variable `red_in_sample`. `red_in_sample` should be equal to the `sum()` of the number of rows in each group with `color` equal to "red."
...
sample_given_urn <- function(...) {
  rep_sample_n(tibble(color = c(rep("red", ...), rep("white", ...))),
               size = 45, reps = 1000, replace = TRUE) %>%
    group_by(replicate) %>%
    summarize(red_in_sample = sum(color == "red"))
}
```
...
sample_given_urn <- function(red_in_urn) {
  rep_sample_n(tibble(color = c(rep("red", red_in_urn), rep("white", 1500 - red_in_urn))),
               size = 45, reps = 1000, replace = TRUE) %>%
    group_by(replicate) %>%
    summarize(red_in_sample = sum(color == "red"))
}
...
```{r posterior17, exercise = TRUE, exercise.setup = "urn", exercise.lines = 20}
sample_given_urn <- function(red_in_urn) {
  rep_sample_n(tibble(color = c(rep("red", red_in_urn), rep("white", 1500 - red_in_urn))),
               size = 45, reps = 1000, replace = TRUE) %>%
    group_by(replicate) %>%
...
tibble(nums = seq(from = 650, to = 1150, by = 25)) %>%
  ...
  unnest(cols = sample_results) %>%
  group_by(nums) %>%
  summarize(num_samples = sum(red_in_sample == 28)) %>%
