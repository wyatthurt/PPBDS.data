...
*Fill in the `ggplot()` function to create a scatterplot with `rating` on the x-axis and `hours` on the y-axis. Is there a relationship between the rating rating of a Harvard course versus the average hours per week of workload?*
...
ggplot(data = qscores, mapping = aes(x = rating)) + geom_histogram()
```
...
ggplot(data = top_five_qscores, mapping = aes(x = department, y = rating)) + geom_boxplot()
```
